...
export async function GET() {
  try {
    ...
    return apiError(error, "Frozen funds error");
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "release") {
      if (typeof body.frozenFundId !== "string" || !body.frozenFundId) {
        return NextResponse.json({ error: "frozenFundId is required" }, { status: 400 });
      }
