...
Під полем вводу постійно висить клавіатура з головними діями (згорнути й розгорнути її
можна кнопкою праворуч від поля вводу) - її не треба викликати
...
Натиски кнопок клавіатури і набрані цифри бот прибирає з чату - інакше вони лягали б
під його блоком, і питання опинялось би вище відповідей. Швидкі текстові записи і
команди лишаються.

Після збереженої операції бот шле коротке «Що далі?» з клавіатурою - саме повідомлення
з нею змушує Telegram її розгорнути, тож меню повертається само, як і після скасування.

Сценарій живе в одному повідомленні: кроки не додають нових, а перемальовують кнопки
на місці, тож чат не засмічується варіантами. Підтвердження операції замінює собою той
самий блок, а наступна операція вже бере нове повідомлення - щоб чек із кнопкою
скасування лишався цілим.
...
- Без валюти - UAH; для робочої витрати - USDT, бо вона зазвичай платиться з гаманця. Підтримуються `UAH`, `грн`, `₴`, `USDT`, `USD`, `$`.
